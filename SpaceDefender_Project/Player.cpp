...
void Player::shoot(int bulletSpeed) {
    // Create a bullet shooting RIGHT from the player's right side
    Bullet *bullet = new Bullet(Bullet::PlayerBullet, bulletSpeed);
    bullet->setPos(x() + pixmap().width(),  // RIGHT side of player sprite
...
void Player::moveUp()
{
    // Move up with screen-edge boundary (272px high scene)
    if (y() > 0)
        setPos(x(), y() - 20);
}
...
{
    // Move down with screen-edge boundary (272px high scene)
