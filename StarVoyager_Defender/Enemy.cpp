...
Enemy::Enemy(int speedMultiplier, int health, QGraphicsItem *parent)
  ...
  startY = rand() % 228;
  setPos(480 + 60, startY); //spawn on right side of screen
}
...
void Asteroid::move() {
  timeAlive++;
  setPos(x() - speed, y());  //asteroid move left
  commonMove();
}
...
void StraightShip::move() {
  timeAlive++;
  setPos(x() - speed, y());  //move left and shoot left
  tryShoot();
  commonMove();
}
...
void StraightShip::tryShoot() {
  if (rand() % 100 < 2) {
    ...
    //bullet spawns on left side of enemy
...
void SineShip::move() {
  ...
  setPos(x() - speed, newY);  //changed to move left and shoot left
  tryShoot();
  commonMove();
}
